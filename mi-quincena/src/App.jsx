...
export default function App() {
  ...
  // El widget de Netlify Identity a veces deja una capa invisible cubriendo
  // toda la página después de iniciar sesión, bloqueando cualquier clic.
  // Por eso forzamos a ocultarla cada vez que el widget se cierra o el
  // usuario ya inició sesión, y a mostrarla solo mientras el modal está abierto.
  useEffect(() => {
    const idt = window.netlifyIdentity;
    if (!idt) { setAuthReady(true); return; }

    const hideWidgetOverlay = () => {
      const el = document.querySelector(".netlify-identity-widget");
    ...
    idt.on("open", showWidgetOverlay);
    idt.on("close", hideWidgetOverlay);
    idt.on("logout", () => { setUser(null); setCategorias([]); setMovimientos([]); setLoading(true); hideWidgetOverlay(); });
    idt.init();
    // por si el evento "close" no llega a tiempo, forzamos el ocultado un momento después también
    const t = setTimeout(hideWidgetOverlay, 800);
    return () => {
      clearTimeout(t);
...
function GridCell({ value, placeholder, onCommit }) {
...
function GridTab({ categorias, periods, cellMap, statsMap, categoriaResumen, updateCell, lastNomina, onAddQuincena }) {
  const groups = [
          ...
          <table className="ledger-table grid-table">
            <thead>
              ...
              <tr>
                ...
                {groups.map((g) => g.cats.map((c) => (
                  <th key={c.id} className={`cat-head group-${g.tipo}`}>{c.nombre}</th>
                )))}
                <th className="cat-head group-Calc">Gasto Seguro</th>
                <th className="cat-head group-Calc">Libre</th>
            ...
            <tbody>
              {periods.map((q) => {
                ...
                return (
                  <tr key={q}>
                    <td className="sticky-col quincena-cell">{fmtQuincena(q)}</td>
                    {groups.map((g) => g.cats.map((c) => {
                    ...
                    <td className="num calc-td">{fmtMoney(stats.gastoSeguro)}</td>
                    <td className={`num calc-td ${stats.libre < 0 ? "neg" : ""}`}>{fmtMoney(stats.libre)}</td>
            ...
            <tfoot>
              <tr>
                ...
                {groups.map((g) => g.cats.map((c) => (
                  <td key={c.id} className="num total-td">{fmtMoney(totalByNombre[c.nombre] || 0)}</td>
                )))}
                <td colSpan={3}></td>
              </tr>
            </tfoot>
